...
/***/ function(module, exports, __webpack_require__) {
	 ...
	 */
	var _generateFactor = function _generateFactor() {
	  ...
	  for (var _i = 0; _i < $scrollBottom.length; _i++) {
	    $scrollBottom[_i].factor = parseFloat($scrollBottom[_i].getAttribute('data-parallax-factor') || settings.factor, 10);
	    $scrollBottom[_i].initialOffset = $scrollBottom[_i].getBoundingClientRect().top;
	  }
	};
	...
	var _callback = function _callback() {
	  ...
	  for (var _i2 = 0; _i2 < $scrollBottom.length; _i2++) {
	    rectObject = $scrollBottom[_i2].getBoundingClientRect();
	    visible = _isElementInViewport($($scrollBottom[_i2]).parent());

	    $scrollBottom[_i2].style.visibility = visible ? 'visible' : 'hidden';
	    if (visible) {
	      offset = rectObject.top - $scrollBottom[_i2].initialOffset;
	      $($scrollBottom[_i2]).css('transform', 'translateY(' + Math.floor(offset / ($scrollBottom[_i2].factor * -1)) + 'px) translateZ(0)');
	    }
	  }
	...
	$.hongkong = function (options) {
	  ...
	  if ($scrollTop.length || $scrollBottom.length) {
	    ...
	    $(window).on('scroll', (0, _throttle3.default)(update));
	  }
	};
